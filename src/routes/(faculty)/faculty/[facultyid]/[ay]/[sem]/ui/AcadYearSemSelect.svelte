<script lang="ts">
    ...
    function handleKeyPress(event: KeyboardEvent) {
        ...
        if (/^[a-z0-9]$/.test(char)) {
            const visibleOpts = isAcadYearDropdownOpen ? acadYearOpts : allSemStrs;
            const matchingOpt = visibleOpts.find((opt) => {
                const optStr = String(opt).toLowerCase();
                return optStr.startsWith(char);
            });

            if (matchingOpt !== undefined) {
                if (isAcadYearDropdownOpen) {
                    stagedAcadYear = matchingOpt as number;
                }
            }
        }
    }
    ...
    $effect(() => {
        if (isAcadYearDropdownOpen || isSemNumDropdownOpen) {
...
<div class="flex items-center gap-2" bind:this={containerRef}>
        ...
        {#if !viewState.isEditing}
            <div
                class="rounded-lg p-1 {isAcadYearDropdownOpen
                    ? 'block'
                    : 'hidden'} absolute z-50 w-full bg-white shadow-lg max-h-80 overflow-y-auto"
            >
